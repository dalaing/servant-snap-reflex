...
import qualified Servant.Snap.Reflex.Comms.Serial as S
import qualified Servant.Snap.Reflex.Comms.Parallel as P
import Servant.Snap.Reflex.Util
...
instance (HasEndpoint api)
      => HasEndpoint (IsSecure :> api) where
  type EndpointInput (IsSecure :> api) =
    IsSecure ': EndpointInput api
  type EndpointOutput (IsSecure :> api) =
    EndpointOutput api

  serve' _ f = \x ->
    serve' (Proxy :: Proxy api) (\xs -> f (GCons x xs))

instance (HasEndpoint api, CollapseList (EndpointInput (IsSecure :> api)))
      => AsServantHost (IsSecure :> api) where
  type ToServantHost (IsSecure :> api) =
    Endpoint
      (Collapsed (EndpointInput (IsSecure :> api)))
      (Snap (Collapsed (EndpointOutput (IsSecure :> api))))
...
{-
instance (HasEndpoint api)
      => HasEndpoint (HttpVersion :> api) where
  type EndpointInput (HttpVersion :> api) =
    HttpVersion ': EndpointInput api
  type EndpointOutput (HttpVersion :> api) =
    EndpointOutput api

  serve' _ f = \x ->
    serve' (Proxy :: Proxy api) (\xs -> f (GCons x xs))

instance (HasEndpoint api, CollapseList (EndpointInput (HttpVersion :> api)))
      => AsServantHost (HttpVersion :> api) where
  type ToServantHost (HttpVersion :> api) =
    Endpoint
      (Collapsed (EndpointInput (HttpVersion :> api)))
      (Snap (Collapsed (EndpointOutput (HttpVersion :> api))))
...
instance (HasEndpoint api)
      => HasEndpoint (RemoteHost :> api) where
  type EndpointInput (RemoteHost :> api) =
    B.ByteString ': EndpointInput api
  type EndpointOutput (RemoteHost :> api) =
    EndpointOutput api

  serve' _ f = \x ->
    serve' (Proxy :: Proxy api) (\xs -> f (GCons x xs))

instance (HasEndpoint api, CollapseList (EndpointInput (RemoteHost :> api)))
      => AsServantHost (RemoteHost :> api) where
  type ToServantHost (RemoteHost :> api) =
    Endpoint
      (Collapsed (EndpointInput (RemoteHost :> api)))
      (Snap (Collapsed (EndpointOutput (RemoteHost :> api))))
...
instance (HasEndpoint api)
      => HasEndpoint (Vault :> api) where
  type EndpointInput (Vault :> api) =
    Vault ': EndpointInput api
  type EndpointOutput (Vault :> api) =
    EndpointOutput api

  serve' _ f = \x ->
    serve' (Proxy :: Proxy api) (\xs -> f (GCons x xs))

instance (HasEndpoint api, CollapseList (EndpointInput (Vault :> api)))
      => AsServantHost (Vault :> api) where
  type ToServantHost (Vault :> api) =
    Endpoint
      (Collapsed (EndpointInput (Vault :> api)))
      (Snap (Collapsed (EndpointOutput (Vault :> api))))

  serve c a s = serve' a $ \g -> do
    res <- liftIO $ reqRes c s (collapse g)
    liftSnap res

instance (KnownSymbol realm, HasEndpoint api)
      => HasEndpoint (BasicAuth realm usr :> api) where
  type EndpointInput (BasicAuth realm usr :> api) =
    usr ': EndpointInput api
  type EndpointOutput (BasicAuth realm usr :> api) =
    EndpointOutput api

  serve' _ f = \x ->
    serve' (Proxy :: Proxy api) (\xs -> f (GCons x xs))

instance (KnownSymbol realm, HasEndpoint api, CollapseList (EndpointInput (BasicAuth realm usr :> api)))
      => AsServantHost (BasicAuth realm usr :> api) where
  type ToServantHost (BasicAuth realm usr :> api) =
    Endpoint
      (Collapsed (EndpointInput (BasicAuth realm usr :> api)))
      (Snap (Collapsed (EndpointOutput (BasicAuth realm usr :> api))))
